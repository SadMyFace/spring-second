    ...
    getCommentListFromServer(bno, page).then(result => {
        ...
        if(result.cmtList.length > 0){
            ...
            for(let cvo of result.cmtList){
                ...
                li += `<button type="button" data-cno=${cvo.cno} class="btn btn-outline-warning btn-sm cmtModBtn">수정</button>`;
                li += `<button type="button" data-cno=${cvo.cno} class="btn btn-outline-danger btn-sm cmtDelBtn">삭제</button>`;
                li += `</li>`;
                ul.innerHTML += li;
            }
        }else{
